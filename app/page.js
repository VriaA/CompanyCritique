...
export default function Home() {
  ...
  return (
    <div className={`flex h-screen ${darkMode ? 'bg-[#111111] text-white' : 'bg-gray-100'}`}>
      ...
      {/* Sidebar */}
      <aside className={`w-60 fixed top-4 left-4 bottom-4 overflow-y-auto p-[1px] bg-gradient-to-b from-[#5661F6] via-[#8091F6] to-[#F6A856] rounded-[30px] transition-transform duration-300 ease-in-out transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
      ...
      {/* Main content area */}
      <div className="flex-1 ml-64 flex flex-col overflow-y-auto relative">
        {/* Welcome screen */}
        <main className={`flex-1 flex flex-col mb-10 items-center justify-center p-4 font-inter font-bold gap-8 ${darkMode ? 'text-white' : ''}`}>
          <Link href="/">
          ...
          <div className="flex space-x-4">
            <button onClick={() => document.querySelector('input').value = "How much is an average software engineer paid at...."} className="relative p-[1px] bg-gradient-to-r from-[#5661F6] via-[#8091F6] to-[#F6A856] rounded-lg">
            ...
            <button onClick={() => document.querySelector('input').value = "Level of diversity among workers at ...."} className="relative p-[1px] bg-gradient-to-r from-[#5661F6] via-[#8091F6] to-[#F6A856] rounded-lg">
              <span className="block bg-white px-3 py-2 rounded-lg text-sm text-left h-full flex items-center">
                Level of diversity among workers at Meta
              </span>
            </button>
            <button onClick={() => document.querySelector('input').value = "Benefits for employees working at ..."} className="relative p-[1px] bg-gradient-to-r from-[#5661F6] via-[#8091F6] to-[#F6A856] rounded-lg">
              <span className="block bg-white px-3 py-2 rounded-lg text-sm text-left h-full flex items-center">
